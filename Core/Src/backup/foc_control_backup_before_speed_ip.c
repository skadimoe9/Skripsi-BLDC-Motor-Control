 ...
 * - Field weakening runs at 500 Hz before the speed PI. It averages the 10 kHz
 ...
 * - Speed mode runs an outer PI every FOC_SPEED_LOOP_DIV current ticks
 *   (10 kHz / 10 = 1 kHz) and outputs an Iq reference.
...
#define FOC_SPEED_KP_A_PER_RADS        0.1f
#endif

#ifndef FOC_SPEED_KI_A_PER_RADS_S
#define FOC_SPEED_KI_A_PER_RADS_S      0.5f
#endif
...
#define FOC_VOLTAGE_LIMIT_FRAC        0.95f // About 66 rad/s mechanical at 38.8 V before field weakening.
#endif
 ...
 * The outer speed PI generates Iq; voltage-limit FW may add negative Id.
 */
...
void foc_control_adc_tick(float dt_s, const adc_sense_data_t *adc, const hall_sensor_data_t *hall)
{
    ...
    if (s_mode == FOC_CONTROL_MODE_SPEED)
    {
        ...
        if (s_fw_tick_div >= FOC_FIELD_WEAKENING_LOOP_DIV)
        {
            ...
            if ((s_fw_enabled != 0U) && (fw_max_current > 0.0f) && (FOC_MOTOR_PSI_WB > 0.0f))
            {
                ...
                if (s_fw_loop_active != 0U)
                {
                    ...
                    float old_fw_int = s_fw_int_a;
                    float id_min = -fw_max_current;

                    s_fw_int_a += FOC_FIELD_WEAKENING_KI_A_PER_V_S * voltage_error * fw_dt_s;
                    float id_cmd = (FOC_FIELD_WEAKENING_KP_A_PER_V * voltage_error) + s_fw_int_a;

                    if (id_cmd < id_min)
                    {
                        id_cmd = id_min;
                        s_fw_int_a = old_fw_int;
                    }
        ...
        if (s_speed_tick_div >= FOC_SPEED_LOOP_DIV)
        {
            ...
            /* Outer speed PI runs at 1 kHz. Output is limited Iq reference;
             * integrator rollback is simple anti-windup when Iq saturates.
             */
             ...
             * of that radius, so the speed PI can only request the remaining Iq.
             */
