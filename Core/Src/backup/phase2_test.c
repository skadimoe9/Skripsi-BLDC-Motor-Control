...
void phase2_test_init(UART_HandleTypeDef *huart)
{
    ...
    uart_print("  foc_start_rpm <rpm> <iq_limit_mA>\r\n");
    uart_print("  foc_set_rpm <rpm> <iq_limit_mA>\r\n");
...
static void phase2_process_cmd(char *cmd)
{
    ...
    else if (strncmp(cmd, "foc_start_rpm ", 14) == 0)
    {
        int32_t rpm = 0;
        int32_t iq_limit_mA = 0;

        if (!phase2_parse_two_i32(&cmd[14], &rpm, &iq_limit_mA))
        {
            uart_print("ERR,FORMAT_foc_start_rpm_rpm_iqLimitmA\r\n");
            return;
        }
        ...
        if ((iq_limit_mA < -5000) || (iq_limit_mA > 6300))
        {
            uart_print("ERR,FOC_IQ_LIMIT_RANGE_100_TO_3000_mA\r\n");
            return;
        }
        ...
        if (foc_control_start_speed((float)rpm, (float)iq_limit_mA * 0.001f))
        {
    ...
    else if (strncmp(cmd, "foc_set_rpm ", 12) == 0)
    {
        int32_t rpm = 0;
        int32_t iq_limit_mA = 0;

        if (!phase2_parse_two_i32(&cmd[12], &rpm, &iq_limit_mA))
        {
            uart_print("ERR,FORMAT_foc_set_rpm_rpm_iqLimitmA\r\n");
            return;
        }
        ...
        foc_control_set_speed_ref((float)rpm, (float)iq_limit_mA * 0.001f);
