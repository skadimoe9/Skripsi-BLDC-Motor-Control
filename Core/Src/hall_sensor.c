 ...
 *
 * Runtime flow:
 * - main.c calls Hall_Init() after TIM5 is ready.
 * - main.c HAL_GPIO_EXTI_Callback() calls Hall_EXTI_Callback() on Hall edges.
 * - Hall_EXTI_Callback() validates transitions and records timing only.
 * - adc_sense.c calls Hall_ControlTick() at 10 kHz before foc_control_adc_tick().
 * - Hall_ControlTick() turns edge timing into electrical angle and speed for FOC.
 */
...
/* State shared between EXTI and the 10 kHz control tick. Access is protected by
 * IRQ masking in foreground reads/updates; Hall_GetDataFromISR() is for ISR use.
 */
...
/* EXTI only sets these pending flags. Hall_ControlTick() consumes them so angle
 * math stays in the regular control tick instead of the GPIO interrupt path.
 */
...
/* Internal helpers: raw-state mapping, transition validation, event queueing,
 * calibrated angle lookup, wrapping, timeout and derived-unit updates.
 */
...
/* Read GPIO Hall pins and pack H1:H2:H3 into a 3-bit value.
 * Used by Hall_Init() and Hall_EXTI_Callback().
 */
...
/* Public validity check used by diagnostics. Valid states are the six entries
 * in s_hall_positive_sequence; 000 and 111 are invalid for this motor.
 */
...
/* Convert a raw state to a short validity string for Hall_PrintState(). */
const char* Hall_StateName(uint8_t state)
{
    return Hall_IsValidState(state) ? "VALID" : "INVALID";
}

/* Initialize Hall reconstruction. Called from main.c once TIM5 is running. */
void Hall_Init(void)
{
    ...
    {
        /* At standstill there is no edge timing yet, so snap to the calibrated
         * middle of the current sector until movement provides edge updates.
         */
        s_hall_index = index;
...
/* Hall edge handler called by main.c HAL_GPIO_EXTI_Callback().
 * Keep this short: validate state/order, update timestamps, and set pending
 * work for Hall_ControlTick(); do not run FOC math here.
 */
void Hall_EXTI_Callback(uint16_t GPIO_Pin, uint32_t timestamp_us)
{
    ...
    {
        /* Illegal raw Hall state, usually wiring/noise/supply issue. */
    ...
    {
        /* First valid state after an invalid period: resync and wait for speed. */
    ...
    {
        /* Multiple EXTI firings without a real state change. */
    ...
    if (direction == 0)
    {
        /* Transition skipped or moved to a non-neighbor sector; resync angle. */
    ...
    {
        /* Reject very short intervals as glitches. */
        s_hall.glitch_count++;
        Hall_ResyncLocked(state, new_index, timestamp_us, 1U);
    ...
    {
        /* Direction reversal: clear timing history and snap to the new edge. */
    ...
    if (s_dt_count >= 3U)
    {
        /* Three Hall sectors equal pi electrical radians, used for speed. */
        s_pending_speed_update = 1U;
    }

    if ((s_valid_transition_count <= 6U) || (s_hall.speed_valid == 0U))
    {
        /* During startup, snap angle to calibrated edges until interpolation is stable. */
        Hall_QueueAngleSnapLocked(HALL_SNAP_EDGE, new_index, direction);
    }

    if ((s_valid_transition_count >= 6U) && Hall_IsCorrectionEdge(new_index, direction))
    {
        /* Periodic correction keeps integrated angle from drifting between edges. */
...
/* 10 kHz Hall estimator tick called from adc_sense.c before FOC.
 * It consumes pending EXTI work, updates speed, applies drift correction, and
 * integrates theta_e_rad forward between physical Hall edges.
 */
void Hall_ControlTick(void)
{
    ...
    {
        s_last_angle_update_us = now_us;
        s_hall.mechanical_rad_s = 0.0f;
        return;
    }

    if (s_pending_angle_snap != HALL_SNAP_NONE)
    {
        /* Snap to a calibrated edge or sector midpoint after startup/resync. */
    ...
    {
        /* dt_sum_us is the rolling time for three 60-degree electrical sectors,
         * i.e. pi electrical radians. omega_e = +/-pi / dt_sum.
         */
    ...
    {
        /* Compare integrated angle to the calibrated edge and apply a small
         * correction speed until the next correction edge.
         */
    ...
    if (dt_us > 0U)
    {
        ...
        if (s_hall.speed_valid != 0U)
        {
            ...
            {
                /* If no edge arrives in time, declare speed invalid and stop extrapolating. */
                s_hall.speed_valid = 0U;
                s_hall.omega_e_rad_s = 0.0f;
                s_hall.omega_corr_rad_s = 0.0f;
                s_hall.mechanical_rad_s = 0.0f;
            ...
            {
                /* Interpolate angle between Hall edges using estimated electrical speed. */
...
/* Foreground-safe snapshot used by can_log.c, foc_control_start_*(), and CLI. */
...
/* ISR/control-path snapshot used by adc_sense.c immediately before FOC tick. */
hall_sensor_data_t Hall_GetDataFromISR(void)
{
    return s_hall;
}

/* Pop one queued transition event. Currently kept for local diagnostics; the
 * periodic CAN telemetry no longer transmits Hall event frames.
 */
...
/* Return how many transition events were dropped because the debug queue was full. */
...
/* UART diagnostic print; currently kept for manual/debug command use. */
void Hall_PrintState(void)
{
    ...
    snprintf(msg, sizeof(msg),
             "Hall: %u%u%u | state=%u | %s | dir=%d | rads_x100=%ld | theta_e_x100=%ld\r\n",
             ...
             (int)hall.direction,
             (long)(hall.mechanical_rad_s >= 0.0f ? (hall.mechanical_rad_s * 100.0f) + 0.5f : (hall.mechanical_rad_s * 100.0f) - 0.5f),
             (long)(hall.theta_e_deg * 100.0f));

    uart_print(msg);
}

/* Check whether the EXTI pin belongs to one of the three Hall inputs. */
static uint8_t Hall_PinIsHall(uint16_t GPIO_Pin)
{
...
/* Convert raw 3-bit Hall state to calibrated sequence index H1..H6. */
static uint8_t Hall_StateToIndex(uint8_t state)
{
...
/* Return +1/-1 for adjacent legal transitions, or 0 for invalid/skipped steps. */
static int8_t Hall_TransitionDirection(uint8_t old_index, uint8_t new_index)
{
...
/* Choose the calibrated edge used for periodic drift correction. */
static uint8_t Hall_IsCorrectionEdge(uint8_t new_index, int8_t direction)
{
    if (direction > 0)
    {
...
/* Clear rolling timing and pending estimator work after resync/reversal. */
static void Hall_ResetTimingLocked(void)
{
    memset(s_dt_us, 0, sizeof(s_dt_us));
    s_dt_pos = 0U;
...
/* Queue an angle snap for Hall_ControlTick(); caller already holds IRQ lock. */
static void Hall_QueueAngleSnapLocked(uint8_t mode, uint8_t index, int8_t direction)
{
...
/* Store a compact transition record for optional debugging. */
static void Hall_QueueTransitionEventLocked(uint8_t previous_state,
                                            uint8_t hall_state,
                                            int8_t direction,
                                            uint32_t timestamp_us)
...
/* Resynchronize after invalid state/transition/glitch or first valid state. */
static void Hall_ResyncLocked(uint8_t state, uint8_t index, uint32_t timestamp_us, uint8_t snap_mid)
{
...
/* Maintain rolling sum of the last three Hall edge intervals for speed estimate. */
static void Hall_AddDtLocked(uint32_t dt_us)
{
    if (s_dt_count < 3U)
    {
...
/* Return calibrated sector midpoint angle for standstill/resync startup. */
static float Hall_SectorMidAngleRad(uint8_t index)
{
    if (index >= 6U)
    {
...
/* Return calibrated transition edge angle, direction-dependent. */
static float Hall_EdgeAngleRad(uint8_t index, int8_t direction)
{
    if (index >= 6U)
    {
...
/* Wrap error angle to [-pi, pi] for correction math. */
static float Hall_WrapMinusPiToPi(float angle_rad)
{
...
/* Timeout threshold for declaring stopped/unknown speed if edges stop arriving. */
static uint32_t Hall_TimeoutFromDtSum(uint32_t dt_sum_us)
{
    uint32_t timeout_us;
...
/* Update derived degrees and mechanical rad/s after angle/speed changes. */
static void Hall_UpdateDerived(void)
{
    ...
    if (s_hall.speed_valid != 0U)
    {
        s_hall.mechanical_rad_s = s_hall.omega_e_rad_s / pole_pairs;
    }
    else
    {
        s_hall.mechanical_rad_s = 0.0f;
