...
int main(void)
{
  ...
  phase2_test_init(&huart6);

  if (adc_sense_start() != HAL_OK)
  {
      Error_Handler();
  }
...
static void MX_TIM1_Init(void)
{
  ...
  sBreakDeadTimeConfig.DeadTime = 86;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : HALL1_Pin HALL2_Pin */
...
{
    phase2_test_uart_rx_callback(huart);
}

void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc)
{
    adc_sense_adc_half_cplt_callback(hadc);
}

void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    adc_sense_adc_cplt_callback(hadc);
}

void HAL_ADC_ErrorCallback(ADC_HandleTypeDef *hadc)
{
   adc_sense_adc_error_callback(hadc);
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if ((GPIO_Pin == HALL1_Pin) ||
        ...
        (GPIO_Pin == HALL3_Pin))
    {
        Hall_EXTI_Callback(GPIO_Pin, __HAL_TIM_GET_COUNTER(&htim5));
    }
}
